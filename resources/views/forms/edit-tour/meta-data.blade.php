    ...
    <div class="h-full max-h-full w-full overflow-y-auto gap-6 gap-y-8 place-content-start grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 p-8">
            ...
            <div role="radiogroup" class="mx-auto flex gap-4 justify-start">
                <div class="flex items-center">
                    <div
                        ...
                        <input id="inbound" value="1" wire:model.live="tour_form.is_inbound" type="radio" name="tour_type"
                ...
                <div class="flex items-center ml-6">
                    <div
                        class="bg-white dark:bg-gray-100 rounded-full w-4 h-4 flex flex-shrink-0 justify-center items-center relative">
                        <input id="outbound" value="0" wire:model.live="tour_form.is_inbound" type="radio" name="tour_type"
                            class="checkbox appearance-none focus:opacity-100 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 focus:outline-none border rounded-full border-gray-400 absolute cursor-pointer w-full h-full checked:border-none" />
                        <div class="check-icon hidden border-4 border-indigo-700 rounded-full w-full h-full z-1"></div>
                    </div>
        ...
        <div>
            ...
            <div role="radiogroup" class="mx-auto flex gap-4 justify-start">
                <div class="flex items-center">
                    <div
                        class="bg-white dark:bg-gray-100 rounded-full w-4 h-4 flex flex-shrink-0 justify-center items-center relative">
                        <input id="type-f" value="F" wire:model.live="tour_form.payment_type" type="radio"
                            name="payment_type"
                            class="checkbox appearance-none focus:opacity-100 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 focus:outline-none border rounded-full border-gray-400 absolute cursor-pointer w-full h-full checked:border-none" />
                        <div class="check-icon hidden border-4 border-indigo-700 rounded-full w-full h-full z-1"></div>
                    </div>
                ...
                <div class="flex items-center ml-6">
                    <div
                        class="bg-white dark:bg-gray-100 rounded-full w-4 h-4 flex flex-shrink-0 justify-center items-center relative">
                        <input id="type-i" value="I" wire:model.live="tour_form.payment_type" type="radio"
                            name="payment_type"
                            class="checkbox appearance-none focus:opacity-100 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 focus:outline-none border rounded-full border-gray-400 absolute cursor-pointer w-full h-full checked:border-none" />
                        <div class="check-icon hidden border-4 border-indigo-700 rounded-full w-full h-full z-1"></div>
                    </div>
        ...
        <div>
            ...
            <div class="flex flex-col max-w-48 items-center gap-2">
                <livewire:location-select :selectedId="$tour_form->origin_id ?? null" placeholder="جستجوی مبدا ..."
                    eventName="origin-item-selected" originMode />
                @error('tour_form.origin_id')
        ...
        <x-textarea title="توضیحات تور" icon_class="comment" wire:model='tour_form.description' placeholder="مسئولیت کنترل پاسپورت از جهت اعتبار و ..."/>
        <x-textarea title="قوانین استرداد" icon_class="book" wire:model='tour_form.return_policy' placeholder="گارانتی و ..."/>
        <x-textarea title="مدارک لازم" icon_class="passport" wire:model='tour_form.required_documents' placeholder="اصل پاسپورت - اصل شناسنامه - اصل کارت ملی"/>
        <x-textarea title="خدمات تور" icon_class="list" wire:model='tour_form.services' placeholder="پرواز رفت و برگشت مستقیم ..."/>
        @if ($tour_form->payment_type === \App\Enums\TourPaymentType::INSTALLMENT)
            <x-textarea title="شرایط اقساطی" icon_class="percentage" wire:model='tour_form.installment_policy' placeholder="اقساط بین 1 تا 4 ماه - کارمزد 4% ..."/>
        @endif
        <div class="col-span-full flex items-center justify-center">
            <button wire:click='submitMetaData' class="px-3 duration-300 py-1 text-white bg-gray-700 rounded-full flex items-center gap-2 font-semibold">
                ذخیره
            </button>
        </div>
    </div>
